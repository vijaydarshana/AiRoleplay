...
export default function SessionHeader({ scenario, timerFormatted, turnCount, onEnd, aiStatus }: Props) {
  ...
  return (
    <header className="bg-slate-950 border-b border-slate-800/80 px-3 sm:px-4 lg:px-6 h-14 sm:h-16 flex items-center gap-2 sm:gap-4 flex-shrink-0 z-20">
      ...
      <div className="flex items-center gap-2 sm:gap-3 flex-shrink-0">
        {/* Timer */}
        <div className="flex items-center gap-1.5 bg-slate-800/80 border border-slate-700/60 rounded-lg px-2.5 sm:px-3 py-2">
          <Clock size={12} className="text-indigo-400" />
          <span className="font-mono-data text-xs sm:text-sm font-bold text-white">{timerFormatted}</span>
        </div>

        {/* Turn count */}
        <div className="hidden sm:flex items-center gap-1.5 bg-slate-800/80 border border-slate-700/60 rounded-lg px-3 py-2">
        ...
        <button
          onClick={onEnd}
          className="flex items-center gap-1.5 bg-red-600 hover:bg-red-500 active:scale-95 text-white text-xs font-bold px-3 sm:px-4 py-2 sm:py-2.5 rounded-lg transition-all duration-150 shadow-sm shadow-red-900/40 touch-manipulation min-h-[40px]"
        >
